...
export class EmailService {
  ...
  // 🔥 ШИНЭЭР НЭМЭВ: Хэрэглэгчийн и-мэйл буруу эсвэл унах үед админд анхааруулга шидэх
  async sendAdminAlert(options: { subject: string; html: string }) {
    try {
      await this.mailerService.sendMail({
        to: 'driftub@gmail.com', // Манай үндсэн админ мэйл
        subject: options.subject,
        html: options.html,
      });
      this.logger.log(`[Alert] Админ руу анхааруулга и-мэйл амжилттай шидлээ.`);
    } catch (err: any) {
      this.logger.error('[Alert] Админ руу анхааруулга явуулахад алдаа гарлаа:', err?.message);
    }
  }
  ...
  async sendOrderConfirmation(dto: any) {
          ...
          <table style="width:100%;border-collapse:collapse;margin-top:12px">
            ...
            .map((it: any) => {
              ...
              const imgTd = imgUrl
                ...
                : `<td style="padding:8px;border-bottom:1px solid #f6f6f6;width:1px"></td>`;
    ...
    const customerHtml = `
      <div style="font-family:Arial,sans-serif;padding:24px;color:#111;max-width:600px;margin:auto">
        ...
        ${renderItems()}
        <p style="margin-top:20px;color:#555">Таны хүргэлт танд 24-36 цаг хооронд хүргэгдэнэ.</p>
    ...
    if (email) {
      tasks.push(
        this.mailerService
          ...
          // ⚠️ Энд хэрэглэгчийн мэйл буруу эсвэл унах үед админд мэдэгдэл илгээнэ, гэхдээ Promise.all-ийг унагахгүйн тулд catch дотор барина
          .catch((err) => {
            this.logger.error(`Хэрэглэгчийн email уналаа (${email}): ${err?.message}`);
            throw err; // Араас нь handle хийхийн тулд throw хийнэ
          }),
      );
    } else {
